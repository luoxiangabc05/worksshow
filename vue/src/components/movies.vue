<template>
    <div class="wrapper">
        ...
        <div class="content">
            ...
            <ul class="infoList">
                <!-- book就是每一本数 -->
                <li v-for="book in booksData.subjects" @click="showDetail(book)">
                    <!-- {{item}} -->
                    <div class="pic"><img :src="'https://images.weserv.nl/?url='+book.images.large"/></div>
                    <div class="pItem">
                        <h4 class="black" ref="catchYou">名称：{{book.title}}</h4>
                        <div class="tags">
                            <!-- 通过v-if控制循环次数 -->
                            <span v-for="tags,i in book.genres" v-if="i<=2">{{tags}}</span>
                        </div>
                        <p v-for="casts,k in book.casts" v-if="k<1">作者：{{casts.name}}</p>
                        <p>评分：{{book.rating.average}}</p>
                        <div class="gray">时间：{{book.year}}</div>
                    </div>
                </li>
            </ul>
        </div>
...
<script>
// 导入详情页的组件
import bdetail from '@/components/moviesDetail'
export default {
    data(){
        return {
            val:"",
    ...
    created(){
        ...
        this.$http.jsonp('https://api.douban.com/v2/movie/search?q=毒液&count='+this.count+'&alt=xd',{ jsonp:"callback"}).then((res)=>{
            console.log(res.body);
            // 将获取的数据赋值给list,让数据响应式
            this.booksData=res.body;
        }).catch((res)=>{
