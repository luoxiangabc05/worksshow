<template>
    <div class="wrapper">
        ...
        <div class="content">
            ...
            <ul class="infoList">
                <!-- book就是每一本数 -->
                <li v-for="book in booksData.musics" @click="showDetail(book)">
                    <!-- {{item}} -->
                    ...
                    <div class="pItem">
                        ...
                        <p>作者：{{book.author[0].name}}</p>
                        <p>评分：{{book.rating.average}}</p>
                        <div class="gray">时间：{{book.attrs.pubdate[0]}}</div>
                    </div>
                </li>
            </ul>
        </div>
...
<script>
// 导入详情页的组件
import bdetail from '@/components/musicDetail'
export default {
    data(){
        return {
            val:"",
    ...
    created(){
        ...
        this.$http.jsonp('https://api.douban.com/v2/music/search?q=赵雷&count='+this.count+'&alt=xd',{ jsonp:"callback"}).then((res)=>{
            console.log(res.body);
            // 将获取的数据赋值给list,让数据响应式
            this.booksData=res.body;
        }).catch((res)=>{
